...
public class fewProduct extends AppCompatActivity implements OnClickItem {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_few_product);
        getSupportActionBar().hide();
        jeweleryAdapters=new jeweleryAdapter(this);
        recyclerView=findViewById(R.id.recproduct);
    ...
    postviewModels.Postsjewlary().observe(this, new Observer<ArrayList<jeweleryModel>>() {
        @Override
        public void onChanged(ArrayList<jeweleryModel> jeweleryModels) {
            if(!jeweleryModels.isEmpty())
            {
                ...
                GridLayoutManager gridLayoutManager
                        = new GridLayoutManager(fewProduct.this,2);
    ...
    private void filterList(String text) {
        ...
        {
            Toast.makeText(fewProduct.this,"No Data Found",Toast.LENGTH_SHORT).show();
        }
        else
        {
    ...
    public void onClickpro(jeweleryModel jeweleryModels) {
        Intent intent =new Intent(fewProduct.this,Details.class);
    ...
    public void onClickpro2(jeweleryModel jewelerymodel1) {
        ...
        try{
            new Thread(new Runnable() {
                @Override
                public void run() {
                    CartDataBase.getInstance(fewProduct.this).Deo().insert(cart);
                }
            }).start();
            Toast.makeText(fewProduct.this, "Added to basket", Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(fewProduct.this, "error while insert", Toast.LENGTH_SHORT).show();
        }
    }
}